...
export class Block {
	...
	static getByName (name) {
		return nameRegister.get(name);
	}
    static getAll () {
        return Array.from(register.values());
    }
	static async parseDefinitions (gl, src, zipOverrides = null) {
		...
		for (const def of definitions) {
            ...
            block.tintable = tintable;
			block.rawTextures = { top, bottom, left, right, front, back, texture, side };
			if (top)
		...
		// Inject breaking stage textures into the atlas
		for (let i = 0; i < 10; i++) {
			textures.add(`destroy_stage_${i}`);
		}

		const atlas = await create3DTexture(gl, textures, 32, zipOverrides);

...
function createVAO (gl, count) {
	...
	const sizes = [3, 3, 3, 1]; // Pos, Norm, Tex, Light
	for (let i = 0; i < count; i++) {
		const buffer = gl.createBuffer();
		buffers.push(buffer);
		gl.enableVertexAttribArray(i);
		gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
		gl.vertexAttribPointer(i, sizes[i] || 3, gl.FLOAT, false, 0, 0);
	}
...
export class Entity {
	constructor (gl) {
		...
		this.lightBuffer = obj.buffers[3];
		this.VAO = obj.vao;
	...
	clone () {
		...
		clone.lightBuffer = this.lightBuffer;
		return clone;
	}
	update () {
	...
	setLightBuffer (lights, dynamic = false) {
		const gl = this.context;
		gl.bindBuffer(gl.ARRAY_BUFFER, this.lightBuffer);
		gl.bufferData(gl.ARRAY_BUFFER, lights, dynamic ? gl.DYNAMIC_DRAW : gl.STATIC_DRAW);
	}
	setPosition (v) {
	...
	generateFromFaces (faces) {
		...
		const lightArray = new Float32Array(count); // Per-vertex light
		const indexArray = new Uint32Array(length * 6);
		...
		for (const face of faces)
		{
			...
			const lights = face[4]; // Optional light values

			if (!verticies || !texture) continue;
			...
			if (lights) {
				lightArray[li++] = lights[0];
				lightArray[li++] = lights[1];
				lightArray[li++] = lights[2];
				lightArray[li++] = lights[3];
			} else {
				lightArray[li++] = 1.0;
				lightArray[li++] = 1.0;
				lightArray[li++] = 1.0;
		...
		this.setLightBuffer(lightArray);

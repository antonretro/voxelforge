...
export class Noise {
	...
	simplex3(xin, yin, zin) {
		...
		const s = (xin + yin + zin) * F3; // Skew the input space
		let i = Math.floor(xin + s);
		let j = Math.floor(yin + s);
		...
		if (v0.x >= v0.y) {
			if (v0.y >= v0.z) { i1=1; j1=0; k1=0; i2=1; j2=1; k2=0; }
			else if (v0.x >= v0.z) { i1=1; j1=0; k1=0; i2=1; j2=0; k2=1; }
			else { i1=0; j1=0; k1=1; i2=1; j2=0; k2=1; }
		} else {
			if (v0.y < v0.z) { i1=0; j1=0; k1=1; i2=0; j2=1; k2=1; }
			else if (v0.x < v0.z) { i1=0; j1=1; k1=0; i2=0; j2=1; k2=1; }
			else { i1=0; j1=1; k1=0; i2=1; j2=1; k2=0; }
		}

		const v1 = { x: v0.x - i1 + G3, y: v0.y - j1 + G3, z: v0.z - k1 + G3 };
		const v2 = { x: v0.x - i2 + 2 * G3, y: v0.y - j2 + 2 * G3, z: v0.z - k2 + 2 * G3 };
		const v3 = { x: v0.x - 1 + 3 * G3, y: v0.y - 1 + 3 * G3, z: v0.z - 1 + 3 * G3 };

		i &= 255; j &= 255; k &= 255;

		let t0 = 0.6 - v0.x**2 - v0.y**2 - v0.z**2;
		if (t0 < 0) n0 = 0;
		else {
			t0 *= t0;
			n0 = t0 * t0 * this.gradient[i + this.permutation[j + this.permutation[k]]].dot(v0);
		}

		let t1 = 0.6 - v1.x**2 - v1.y**2 - v1.z**2;
		if (t1 < 0) n1 = 0;
		else {
			t1 *= t1;
			n1 = t1 * t1 * this.gradient[i + i1 + this.permutation[j + j1 + this.permutation[k + k1]]].dot(v1);
		}

		let t2 = 0.6 - v2.x**2 - v2.y**2 - v2.z**2;
		if (t2 < 0) n2 = 0;
		else {
			t2 *= t2;
			n2 = t2 * t2 * this.gradient[i + i2 + this.permutation[j + j2 + this.permutation[k + k2]]].dot(v2);
		}

		let t3 = 0.6 - v3.x**2 - v3.y**2 - v3.z**2;
		if (t3 < 0) n3 = 0;
		else {
			t3 *= t3;
			n3 = t3 * t3 * this.gradient[i + 1 + this.permutation[j + 1 + this.permutation[k + 1]]].dot(v3);
		}

		return 32 * (n0 + n1 + n2 + n3) + 0.5;
	}
